...
- dedicated thread handles, a bounded default pool, tasks, structured groups, channels,
  mutexes, events, integer atomics, and seeded or exactly-once initialization;
...
- bounded-executor asynchronous files, sockets, cancellable child processes,
  and timers integrated with Windows overlapped events, readiness, waitable
  timers, and process handles without an unbounded thread-per-operation implementation.
...
non-Windows async backend. File transfer is overlapped; socket/process
coordination is worker-blocking rather than a general IOCP dispatcher. Process
tasks inherit standard streams rather than capturing them, only the bounded
default pool is public, and task groups are constructed from an existing finite
task array. Multi-platform event systems remain Phase 19 work.
