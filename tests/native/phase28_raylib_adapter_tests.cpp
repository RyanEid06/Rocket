...
void graphicsOnlyAssetStoreCycle() {
  ...
  expect(rlv_asset_texture_load(store, textureName, texturePath) > 0,
         "load graphics resources without audio");
  const int64_t texture = rlv_asset_texture_lookup(store, textureName);
  expect(texture > 0 && rlv_asset_texture_borrow(texture) > 0,
         "borrow graphics resources from a graphics-only store");
