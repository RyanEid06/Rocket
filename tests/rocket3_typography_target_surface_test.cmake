if(NOT DEFINED ROCKETC OR NOT DEFINED SELFHOST OR NOT DEFINED SOURCE_DIR OR
   NOT DEFINED WORK)
  message(FATAL_ERROR "WP22 typography target-surface test is missing required arguments")
endif()

file(REMOVE_RECURSE "${WORK}")
set(package "${SOURCE_DIR}/examples/raylib_showcase")
set(public_package "${SOURCE_DIR}/tests/fixtures/rocket3_typography_public_package")
foreach(compiler_name IN ITEMS stage0 selfhost)
  if(compiler_name STREQUAL "stage0")
    set(compiler "${ROCKETC}")
  else()
    set(compiler "${SELFHOST}")
  endif()
  foreach(target IN ITEMS windows-x64 linux-x64 linux-arm64 macos-arm64)
    execute_process(COMMAND "${compiler}" check "${package}" --target "${target}"
      WORKING_DIRECTORY "${SOURCE_DIR}"
      RESULT_VARIABLE check_result OUTPUT_VARIABLE check_output ERROR_VARIABLE check_error)
    if(NOT check_result EQUAL 0)
      message(FATAL_ERROR "WP22 typography ${compiler_name} ${target} surface failed:\n${check_output}${check_error}")
    endif()
    execute_process(COMMAND "${compiler}" check "${public_package}" --target "${target}"
      WORKING_DIRECTORY "${SOURCE_DIR}"
      RESULT_VARIABLE public_result OUTPUT_VARIABLE public_output ERROR_VARIABLE public_error)
    if(NOT public_result EQUAL 0)
      message(FATAL_ERROR "WP22 canonical typography ${compiler_name} ${target} surface failed:\n${public_output}${public_error}")
    endif()
  endforeach()
endforeach()
message(STATUS "WP22 typography source surface passed in stage0/self-host for four targets")
